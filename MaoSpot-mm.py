...
async def cancel_order(exchange, symbol, positionSide:str=None, refCOID=None):
    ...
    return
#-------------------------------------------------------------------------------
async def spot_TPSL(exchange, symbol, amount, priceEntry, priceTP, priceSL, refCOID=None):
    logger.debug(f'{symbol} spot_TPSL PriceEntry:{priceEntry}')
    await spot_TP(exchange, symbol, amount, priceTP, refCOID)
    await spot_SL(exchange, symbol, amount, priceSL, refCOID)
    return
async def spot_TP(exchange, symbol, amount, priceTP, refCOID=None):
    order = {}
    params = {}
    if refCOID:
...
def cal_tpsl(symbol, amount, priceEntry, costAmount):
    ...
    return (priceTP, priceSL, cfg_callback)
            ...
            if tradeMode == 'on' and balance_entry[marginType] > config.not_trade \
                ...
                if amount <= 0.0:
                    ...
                    if TPSLMode:
                        ...
                        await spot_TPSL(exchange, symbol, amount, priceEntry, priceTP, priceSL, refClientOrderId)
        ...
        for position in mm_positions:
            ...
            for coid in position_infos.keys():
                if 'sl_price' in position_infos[coid].keys():
                    if config.tpsl_mode and position_infos[coid]['sl_price'] <= marketPrice:
                    ...
                    if config.tpsl_mode and position_infos[coid]['tl_price'] >= marketPrice:
        ...
        for position in tl_positions:
            ...
            for coid in position_infos.keys():
                if 'sl_price' in position_infos[coid].keys():
                    ...
                    elif config.tpsl_mode and position_infos[coid]['sl_price'] <= closePrice:
